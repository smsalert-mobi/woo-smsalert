...
class WooSmsAlert {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Xc_Woo_Twilio_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		if ( defined( 'XC_WOO_TWILIO_VERSION' ) ) {
			$this->version = XC_WOO_TWILIO_VERSION;
		} else {
			$this->version = '1.0.0';
		}
	...
	private function load_dependencies() {
		...
		$this->loader = new SmsAlert_Loader();
	...
	private function set_locale() {
		...
		$plugin_i18n = new Xc_Woo_Twilio_i18n();
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_filter( 'plugin_action_links_' . XC_WOO_TWILIO_BASENAME, $plugin_admin, 'plugin_action_links' );
		
		$this->loader->add_filter( 'woocommerce_settings_tabs_array', $plugin_admin, 'add_settings_tab', 50 );
		
		$this->loader->add_action( 'woocommerce_sections_xc_woo_twilio', $plugin_admin, 'output_sections'  );
		
		$this->loader->add_action( 'woocommerce_settings_tabs_xc_woo_twilio', $plugin_admin, 'settings_tab' );
		$this->loader->add_action( 'woocommerce_update_options_xc_woo_twilio', $plugin_admin, 'update_settings' );
		
		$this->loader->add_action( 'add_meta_boxes', $plugin_admin, 'add_order_meta_box' );
		$this->loader->add_action( 'wp_ajax_xc_woo_twilio_order_message_load_message', $plugin_admin, 'woo_smsalert_order_message_load_message' );
		$this->loader->add_action( 'wp_ajax_xc_woo_twilio_order_message_send_message', $plugin_admin, 'woo_order_message_send_message' );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new Xc_Woo_Twilio_Public( $this->get_plugin_name(), $this->get_version() );
	...
	private function define_notification_hooks(){
		...
		if('yes' === get_option('xc_woo_twilio_neworder', '')){
		...
		$this->loader->add_action( 'woocommerce_order_status_changed', $notifications, 'notification_after_order', 10, 4 );
		
		$this->loader->add_action( 'woocommerce_new_customer_note', $notifications, 'new_customer_note', 10 );
		
		$this->loader->add_action( 'xc_woo_twilio_custom_message', $notifications, 'custom_order_message', 10, 2 );
	}
	 ...
	 *
	 * @since     1.0.0
	 * @return    Xc_Woo_Twilio_Loader    Orchestrates the hooks of the plugin.
	 */
