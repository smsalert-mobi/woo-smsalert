...
class WooSmsAlert_Notifications
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        $this->sid             = get_option('woo_smsalert_sid', '');
        $this->token           = get_option('woo_smsalert_token', '');
        $this->whatsapp_sender = get_option('woo_smsalert_whatsapp_number', '');
        $this->sms_sender      = get_option('woo_smsalert_sms_number', '');
    ...
    public function send_message($message, $placeholders, $number)
    {
        if (empty($message) || empty($number)) {
            return ['error'   => esc_attr__('Empty phone number or Empty message', 'woo_smsalert'),
                    'status'  => 'faild',
                    'message' => $message,
            ];
        }
        $message_type = get_option("woo_smsalert_default_type", 'sms');
